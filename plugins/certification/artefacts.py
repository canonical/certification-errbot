...
def artefacts_by_user_handle(
    ...
    for artefact in artefacts_response:
        ...
        if not artefact.reviewers and not artefact.due_date:
            continue

        reviewer_handles: List[str] = []
        for reviewer in artefact.reviewers:
            if not reviewer.launchpad_email:
                continue
            user_details = get_user_handle(reviewer.launchpad_email)
            if user_details:
                handle = user_details["username"]
                if handle not in reviewer_handles:
                    reviewer_handles.append(handle)

        if not reviewer_handles:
            reviewer_handles = ["No assignee"]

        if assigned_to_filter:
            reviewer_handles = [
                handle
                for handle in reviewer_handles
                if assigned_to_filter.lower() == handle.lower()
            ]
            if not reviewer_handles:
                continue
        ...
        for handle in reviewer_handles:
            if handle not in artefacts_by_user:
                artefacts_by_user[handle] = []
            artefacts_by_user[handle].append(artefact)
...
def pending_artefacts_by_user_handle() -> Dict[str | None, List[ArtefactResponse]]:
    ...
    with test_observer_client:
        ...
        for artefact in r.parsed:
            ...
            reviewer_handles: List[str | None] = []
            for reviewer in artefact.reviewers:
                if not reviewer.launchpad_email:
                    continue
                user_details = get_user_handle(reviewer.launchpad_email)
