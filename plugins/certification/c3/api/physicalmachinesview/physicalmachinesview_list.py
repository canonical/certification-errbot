...
from ...models.physical_machine_view import PhysicalMachineView
...
def _get_kwargs(
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/api/v2/physicalmachinesview/",
        "params": params,
    }
...
) -> Optional[list["PhysicalMachineView"]]:
    if response.status_code == 200:
        response_200 = []
        _response_200 = response.json()
        for (
            componentsschemas_paginated_physical_machine_view_list_item_data
        ) in _response_200:
            componentsschemas_paginated_physical_machine_view_list_item = (
                PhysicalMachineView.from_dict(
                    componentsschemas_paginated_physical_machine_view_list_item_data
                )
            )
...
def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[list["PhysicalMachineView"]]:
    return Response(
