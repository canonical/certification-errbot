...
) -> Optional[list["ReadPhysicalMachine"]]:
    if response.status_code == 200:
        response_200 = []
        _response_200 = response.json()
        for (
            componentsschemas_paginated_read_physical_machine_list_item_data
        ) in _response_200:
            componentsschemas_paginated_read_physical_machine_list_item = (
                ReadPhysicalMachine.from_dict(
                    componentsschemas_paginated_read_physical_machine_list_item_data
                )
            )
...
def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[list["ReadPhysicalMachine"]]:
    return Response(
