...
from mattermost_api import get_user_by_name, get_mattermost_handle_by_email, mattermost_token, mattermost_base_url
...
def get_email_from_mattermost_handle_api(mattermost_handle: str) -> Optional[str]:
    ...
    try:
        user_details = get_user_by_name(mattermost_token, mattermost_base_url, mattermost_handle)
        email = user_details.get('email')
        
        if email:
            # Cache the result
            mattermost_email_cache[mattermost_handle] = email
            logger.info(f"Found email {email} for Mattermost handle {mattermost_handle}")
            return email
        else:
            logger.warning(f"No email found for Mattermost handle {mattermost_handle}")
    except Exception as e:
        logger.error(f"Failed to get email for Mattermost handle {mattermost_handle}: {str(e)}")
...
def get_email_from_github_username(github_username: str) -> Optional[str]:
    ...
    try:
        ...
        if conn.entries:
            ...
            if email:
                logger.info(f"Found email {email} for GitHub username {github_username}")
                return email
            else:
        ...
        logger.error(f"LDAP lookup failed for GitHub username {github_username}: {str(e)}")
        return None
    finally:
        if 'conn' in locals():
            conn.unbind()
...
def get_mattermost_handle_from_github_username(github_username: str) -> Optional[str]:
    ...
    mattermost_handle = get_mattermost_handle_by_email(mattermost_token, mattermost_base_url, email)
    if mattermost_handle:
        logger.info(f"Found Mattermost handle {mattermost_handle} for GitHub username {github_username}")
        return mattermost_handle
    else:
        logger.warning(f"Could not find Mattermost handle for email {email} (GitHub user: {github_username})")
        return None
